...
//				updateEnemyMove(e);
	...
	private void removeLives(Enemy e) {
		switch(e.getEnemyType()) {
		case ORC:
			playing.removeLives(2);
			break;
		case BAT:
			playing.removeLives(1);
			break;
		case KNIGHT:
			playing.removeLives(3);
			break;
		case WOLF:
			playing.removeLives(1);
			break;
	...
	public void spawnBossEnemy(int bossEnemy, int health) {
		addBoss(bossEnemy, health);
	}
	...
	public void addBoss(int enemyType, int health) {
		int x = start.getxCord() * 32;
		int y = start.getyCord() * 32;

		switch (enemyType) {
		case ORC:
			enemies.add(new Orc(x, y, 0, health, this));
			break;
		case BAT:
			enemies.add(new Bat(x, y, 0, health, this));
			break;
		case KNIGHT:
			enemies.add(new Knight(x, y, 0, health, this));
			break;
		case WOLF:
			enemies.add(new Wolf(x, y, 0, health, this));
			break;
		}
	}
	...
	private void drawHealthBar(Enemy e, Graphics g) {
		...
		g.fillRect((int)e.getX(), (int)e.getY() - 5, HPbarWidth, 3);
		g.setColor(Color.red);
		g.fillRect((int)e.getX(), (int)e.getY() - 5, getNewBarWidth(e), 3);
	...
	public int getAmountOfAliveEnemies(int currentWave) {
		int enemiesLeft = playing.getWaveManager().getNumberOfEnnemies(currentWave);
		for(Enemy e : enemies)
			if(!e.isAlive())
				enemiesLeft--;
		return enemiesLeft;
